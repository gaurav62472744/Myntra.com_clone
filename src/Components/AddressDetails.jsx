...
import {
  Box, FormLabel, Button, Input, Radio, Spacer, Text, Modal,
  ModalOverlay,
  ModalContent,
  ModalHeader,
  ModalFooter,
...
import { addAddress, deleteAddress, getAddres } from '../Redux/CartAddress/action'
...
export const AddressDetails = () => {
  ...
  const handleForm = () => {
    ...
    dispatch(addAddress(name, mobile, pinCode, address, locality, city, state)).then((res) => {
      setcity('')
      setaddress('')
      setlocality('')
      setmobile('')
    ...
    <Box mt='20px' w={{ base: '95%', md: '95%', lg: '650px' }} mr='50px'>
      <Box display={'flex'} justifyContent='space-between'>
        ...
        <Box>
          ...
          <Button fontSize='13px' size='sm' bg='white' border='1px solid black' onClick={onOpen}>ADD NEW ADDRESS</Button>
...

      {
        addresses.map((el, index) => {
          return <Box key={index} mt='10px' w={'100%'} p={{ base: 0, md: 0, lg: 3 }} boxShadow='  rgba(0, 0, 0, 0.05) 0px 6px 24px 0px, rgba(0, 0, 0, 0.08) 0px 0px 0px 1px;'>
            ...
            <Box ml='30px' textAlign={'left'}>
              <Text fontSize={'14px'} >{el.address},{el.locality} {el.city}</Text>
              <Text fontSize={'14px'}>{el.city},{el.state}--{el.pinCode}</Text>
              <Box display={'flex'}>
              ...
              <Box mt='10px' display={'flex'} gap={4}>
                <Button fontSize='13px' size='sm' bg='white' border='1px solid black' onClick={() => dispatch(deleteAddress(el.id)).then(res => dispatch(getAddres()))}>Remove</Button>
                <Button fontSize='13px' size='sm' bg='white' border='1px solid black'>Edit</Button>
              </Box>
            </Box>
          </Box>
        })
      ...
      <Box mt='40px' w={'100%'} p={{ base: 0, md: 0, lg: 2 }} boxShadow='   rgba(0, 0, 0, 0.05) 0px 6px 24px 0px, rgba(0, 0, 0, 0.08) 0px 0px 0px 1px;'>
        <FormLabel onClick={onOpen} fontWeight={'bold'} color='#FF3F6C'>+ Add New Address</FormLabel>
        <Modal
          initialFocusRef={initialRef}
          finalFocusRef={finalRef}
          isOpen={isOpen}
